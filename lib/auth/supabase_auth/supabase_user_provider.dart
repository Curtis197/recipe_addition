...
class AkeliCreatorSupabaseUser extends BaseAuthUser {
  AkeliCreatorSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> akeliCreatorSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AkeliCreatorSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}

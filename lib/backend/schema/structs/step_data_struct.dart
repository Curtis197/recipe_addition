...
class StepDataStruct extends FFFirebaseStruct {
  StepDataStruct({
    bool? success,
    String? message,
    int? recipeId,
  ...
  static StepDataStruct fromMap(Map<String, dynamic> data) => StepDataStruct(
  ...
  static StepDataStruct? maybeFromMap(dynamic data) =>
      data is Map ? StepDataStruct.fromMap(data.cast<String, dynamic>()) : null;
  ...
  static StepDataStruct fromSerializableMap(Map<String, dynamic> data) =>
      StepDataStruct(
  ...
  @override
  String toString() => 'StepDataStruct(${toMap()})';

  @override
  bool operator ==(Object other) {
    return other is StepDataStruct &&
...
StepDataStruct createStepDataStruct({
  bool? success,
  String? message,
  int? recipeId,
  String? recipeFieldUsed,
...
StepDataStruct? updateStepDataStruct(
  StepDataStruct? stepData, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    stepData
...
void addStepDataStructData(
  ...
  if (stepData.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && stepData.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final stepDataData = getStepDataFirestoreData(stepData, forFieldValue);
  final nestedData = stepDataData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = stepData.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getStepDataFirestoreData(
  StepDataStruct? stepData, [
  bool forFieldValue = false,
]) {
  if (stepData == null) {
  ...
  final firestoreData = mapToFirestore(stepData.toMap());

  // Handle nested data for "operations" field.
  addOperationsStructData(
    firestoreData,
    stepData.hasOperations() ? stepData.operations : null,
    'operations',
    forFieldValue,
  );

  // Add any Firestore field values
  stepData.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getStepDataListFirestoreData(
  List<StepDataStruct>? stepDatas,
) =>
    stepDatas?.map((e) => getStepDataFirestoreData(e, true)).toList() ?? [];
